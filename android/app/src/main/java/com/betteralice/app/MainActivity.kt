...
/**
 * Single-activity host. Loads alice.yandex.ru inside a full-screen WebView and injects the BDS
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        assetLoader =
                WebViewAssetLoader.Builder()
                        .setDomain(getString(R.string.bap_asset_authority))
                        .addPathHandler("/", WebViewAssetLoader.AssetsPathHandler(this))
                        .build()

        val isSystemDark = (resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) ==
            Configuration.UI_MODE_NIGHT_YES
        // Yandex Alice's persisted theme drives colours; system dark mode is only the first-launch
        ...
        webView.loadUrl(getString(R.string.bap_target_url))
    ...
    /**
     * Routes VIEW intents (e.g. deep links to alice.yandex.ru) into the existing WebView session
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        if (url.startsWith("https://alice.yandex.ru")) {
            webView.loadUrl(url)
        }
    }
    ...
    private fun injectBdsScripts(view: WebView) {
        ...
        // Detection is text-based so it survives Yandex Alice's dynamic class renames.
        ...
        // Hides the "Download mobile App" item from Yandex Alice's settings dropdown.
        // Uses the same .ds-dropdown-menu / .ds-dropdown-menu-option__label structure
        // as SidebarMenuInjector. Update TARGET if Yandex Alice renames the item (e.g. i18n).
        val hideDrawerItem =
                """
            (function () {
    ...
    companion object {
        ...
        // system bars. Approximates Yandex Alice's own page backgrounds so the status/nav bar region
