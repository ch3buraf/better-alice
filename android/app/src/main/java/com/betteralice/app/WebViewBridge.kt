...
class WebViewBridge(
    ...
    /**
     * Returns the last Yandex Alice page theme written by the extension's theme.js via
     ...
     * "__bap_native_files_picked_<requestId>".
     */
    @JavascriptInterface
    ...
    private fun deliverPickResult(requestId: String, payload: JSONObject) {
        ...
        val script =
                "(function(){try{var d=JSON.parse($payloadLiteral);" +
                        "window.dispatchEvent(new CustomEvent('__bap_native_files_picked_$safeId'," +
                        "{detail:d}));}catch(e){}})();"
    ...
    fun getAssetUrl(relativePath: String?): String {
        val authority = context.getString(R.string.bap_asset_authority)
     ...
     *
     * Supported types: bap-fetch-url -> { ok, html } bap-fetch-github-zip -> { ok, base64,
     * status?, authRejected? } bap-fetch-github-commits -> { ok, commits, status?,
     * authRejected?, rateLimited? } bap-get-youtube-transcript -> { ok: false, error: "..." }
     *
    ...
    fun fetch(payloadJson: String?): String {
        ...
        try {
            ...
            when (val type = payload.optString("type")) {
                "bap-fetch-url" -> handleFetchUrl(payload, response)
                "bap-fetch-github-zip" -> handleFetchGithubZip(payload, response)
                "bap-fetch-github-commits" -> handleFetchGithubCommits(payload, response)
                "bap-get-youtube-transcript" -> {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "bap_storage"
        ...
        // polyfill routes chrome.storage.local.set({ bap_page_is_dark: ... }) through setStorage,
        // so getLastKnownIsDark() and the polyfill share the same SharedPreferences key.
        internal const val KEY_LAST_PAGE_DARK = "bap_page_is_dark"
