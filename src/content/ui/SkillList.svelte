...
<div class="bap-section-title">
  <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
    ...
    <div style="display: flex; gap: 6px;">
      <button type="button" class="bap-btn-outlined" onclick={exportSkills}>
        Export
      </button>
...
<label class="bap-label" for="bap-skill-upload">Загрузить навык (.md)</label>
<input
  id="bap-skill-upload"
  type="file"
  accept=".md"
...
<div id="bap-skill-list" class="bap-list">
  {#if skills.length === 0}
    <p class="bap-empty">Навыки не загружены.</p>
  {:else}
    {#each skills as skill (skill.id)}
      ...
      {:else}
        <div class="bap-skill-item">
          ...
          <div style="display: flex; gap: 6px;">
            <button type="button" class="bap-btn-outlined" style="font-size: 11px; padding: 4px 8px;" onclick={() => startEdit(skill)}>
              Edit
            </button>
            <button type="button" class="bap-btn-danger" onclick={() => deleteSkill(skill.id)}>
              Delete
            </button>
          </div>
        </div>
