...
<div class="bap-manager-header">
  <button
    type="button"
    class="bap-back-btn"
    onclick={view === "detail" ? goBack : onback}
  >
    ← {view === "detail" ? "Проекты" : "Back"}
  </button>
  <div class="bap-section-title" style="margin: 0; padding: 0; border: none;">
    {view === "detail" ? selectedProject?.name || "Project" : "Manage Проекты"}
  </div>
</div>
...
{#if view === "list"}
  <div style="margin-bottom: 8px;">
    ...
    {:else}
      <div class="bap-inline-editor">
        ...
        />
        <textarea
          class="bap-input bap-desc-input"
          bind:value={createDescription}
          placeholder="Description (optional)"
        ></textarea>
        {#if createError}
          <p class="bap-field-error">{createError}</p>
        {/if}
        <div class="bap-editor-actions">
          <button
            type="button"
            class="bap-btn-outlined"
            onclick={() => {
              showCreateForm = false;
              createError = "";
            }}
          ...
          <button type="button" class="bap-btn" onclick={handleCreate}
            >Create</button
          >
        </div>
      </div>
  ...
  <div class="bap-list">
    {#if projects.length === 0}
      <p class="bap-empty">Проектов пока нет.</p>
    {:else}
      {#each projects as project (project.id)}
        <div
          class="bap-skill-item"
        ...
        >
          <div style="flex: 1; min-width: 0;">
            ...
            {#if project.description}
              <div class="bap-project-desc-snippet">
                {project.description}
              </div>
            {/if}
          </div>
...
{:else if view === "detail" && selectedProject}
  <!-- Name + Description -->
  <div class="bap-field-group">
    <label class="bap-label" for="pm-name">Name</label>
    <input
      id="pm-name"
      class="bap-input"
      bind:value={editName}
  ...
  <div class="bap-field-group">
    <label class="bap-label" for="pm-desc">Description</label>
    <textarea
      id="pm-desc"
      class="bap-input"
  ...
  <div class="bap-field-group">
    <label class="bap-label" for="pm-instructions">Кастомные инструкции</label>
    <textarea
      id="pm-instructions"
      class="bap-input"
  ...
  <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
    <div class="bap-subsection-title" style="margin: 0;">Files</div>
  ...
  {#if fileError}
    <p class="bap-field-error">{fileError}</p>
  {/if}

  <div class="bap-list" style="margin-bottom: 8px;">
    {#if projectFiles.length === 0}
      <p class="bap-empty" style="font-size: 11px;">No files added yet.</p>
    {:else}
  ...
  {#if showDeleteConfirm}
    <div class="bap-confirm-box bap-confirm-danger">
      <p class="bap-confirm-text">
      ...
      <div class="bap-editor-actions">
        <button
          type="button"
          class="bap-btn-outlined"
          onclick={() => (showDeleteConfirm = false)}>Отмена</button
        >
        <button
          type="button"
          class="bap-btn-danger"
          onclick={confirmDeleteProject}>Удалить проект</button
        >
      </div>
    </div>
  {:else}
...
<style>
  .bap-manager-header {
    ...
    margin-bottom: 14px;
    padding-bottom: 10px;
    border-bottom: 1px solid var(--bap-border, rgba(0, 0, 0, 0.08));
  }
  .bap-back-btn {
    background: none;
    border: none;
    cursor: pointer;
    color: var(--bap-accent, #4f6ef7);
    font-size: 12px;
    padding: 2px 0;
    flex-shrink: 0;
  }
  ...
  .bap-confirm-box {
    background: var(--bap-surface, #f8f8f8);
    border: 1px solid var(--bap-border, #ddd);
    border-radius: 6px;
    padding: 10px 12px;
    margin-bottom: 8px;
  }
