...
export function collectMessages(filterIds = null) {
  ...
  for (const node of nodes) {
    ...
    const id = node.getAttribute("data-bap-msg-id");
...
/**
 * Extract data from bap-specific UI components in the message.
 */
export function extractBdsCards(node) {
  const cards = [];
  
  // ── Visualizer Card ──
  const visualizer = node.querySelector(".bap-visualizer-card");
  if (visualizer) {
    cards.push({
      type: "visualizer",
      title: visualizer.querySelector(".bap-visualizer-header h4")?.textContent?.trim() || "Visualizer",
      details: visualizer.querySelector(".bap-visualizer-header p")?.textContent?.trim() || "Interactive Simulation"
    });
  }

  // ── PowerPoint Card ──
  const pptx = node.querySelector(".bap-pptx-card");
  if (pptx) {
    cards.push({
      type: "tool",
      title: pptx.querySelector(".bap-pptx-details h4")?.textContent?.trim() || "PowerPoint",
      details: pptx.querySelector(".bap-pptx-details p")?.textContent?.trim() || "Presentation"
    });
  }

  // ── Generic Tool Card (Safety Fallback) ──
  const toolCard = node.querySelector(".bap-tool-card");
  if (toolCard && !pptx) {
    cards.push({
      type: "tool",
...
export function getSessionTitle() {
  const title = document.title || "Yandex Alice Session";
  // Remove " - Yandex Alice" suffix if present
  return title.replace(/ - Yandex Alice$/i, "").trim();
}
...
function formatAssistantContent(content) {
  ...
  let text = content.replace(/<(BDS|BetterAlice):[\s\S]*?<\/(BDS|BetterAlice):[\s\S]*?>/gi, "").trim();
  
  // Extra layer: remove Yandex Alice UI artifacts that might have survived extraction
  const noisePatterns = [
...
<body>
  <div class="container">
    <header>
      <span class="logo-text">Better Alice</span>
    ...
    <footer>
      Document generated via Better Alice Browser Extension
    </footer>
  </div>
</body>
</html>
...
export async function exportToImage(messages, title, dark, fileName) {
  ...
  container.className = `bap-export-container ${dark ? "dark" : ""}`;
  ...
  container.innerHTML = `
    <div style="font-family: 'Inter', sans-serif; line-height: 1.6; color: ${dark ? "#e5e7eb" : "#111827"};">
      <header style="margin-bottom: 40px; padding-bottom: 20px; border-bottom: 1px solid ${dark ? "#262626" : "#e5e7eb"}; display: flex; justify-content: space-between; align-items: center;">
        <span style="font-weight: 800; font-size: 18px; color: #10a37f;">Better Alice</span>
...
<head>
  ...
  <title>${title} | Better Alice Export</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  ...
  <style>
    ...
    * { box-sizing: border-box; transition: background-color 0.3s, color 0.3s; }
    ...
    .bap-card.visualizer .bap-card-icon {
      color: var(--primary);
...
<body>
  ...
  <div class="container">
    ...
    ${messages.map(msg => `
      <div class="message ${msg.role}">
        ...
        ${msg.bdsCards && msg.bdsCards.length ? `
          <div class="bap-cards">
            ${msg.bdsCards.map(card => `
              <div class="bap-card ${card.type}">
                <div class="bap-card-icon">
                  ${card.type === 'visualizer' ? '👁️' : '🛠️'}
                </div>
                <div class="bap-card-info">
                  <div class="bap-card-title">${card.title}</div>
                  <div class="bap-card-details">${card.details || card.status || ""}</div>
                </div>
              </div>
            `).join("")}
          </div>
    ...
    <footer>
      Exported via Better Alice Browser Extension
    </footer>
  </div>
