    ...
    try {
      ...
      if (url.includes("/api/v0/chat_session/fetch_page")) {
        ...
        cloned.json().then(data => {
          window.dispatchEvent(new CustomEvent("bap:session-data", { detail: JSON.stringify(data) }));
        }).catch(() => {});
        return response;
      }
      ...
      try {
        ...
        if (response && response.status >= 500) {
          window.dispatchEvent(new CustomEvent("bap:network-error", {
      ...
      console.warn("[BetterAlice] Request patch failed:", error);
      return originalFetch.apply(this, arguments);
    }
  };
}
...
function emitTokenUsage(inputTokens, outputTokens, modelName) {
  ...
  window.dispatchEvent(new CustomEvent("bap:token-usage", {
    detail: JSON.stringify({
